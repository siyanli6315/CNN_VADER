...
model = Sequential()
model.add(Embedding(input_dim=len(vocabulary),output_dim=20,input_length=56,weights=embedding_weights))
model.add(Dropout(0.7,input_shape=(56,20)))
model.add(graph)
model.add(Dense(100))
...
model.fit(x_shuffled, y_shuffled, batch_size=32,nb_epoch=100,shuffle=True,validation_split=0.1,verbose=2)
